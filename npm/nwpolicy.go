...
func (npMgr *NetworkPolicyManager) AddNetworkPolicy(npObj *networkingv1.NetworkPolicy) error {
	...
	addIpsetEntries("in", npObj.ObjectMeta.Name, ingressIPCidrs, ipsMgr)
	addIpsetEntries("out", npObj.ObjectMeta.Name, egressIPCidrs, ipsMgr)
	iptMgr := allNs.iptMgr
...
func addIpsetEntries(ingressOrEgress string, policyName string, ipsetEntries [][]string, ipsMgr *ipsm.IpsetManager) {
...
func (npMgr *NetworkPolicyManager) DeleteNetworkPolicy(npObj *networkingv1.NetworkPolicy) error {
	...
	_, _, _, _, _, iptEntries := translatePolicy(npObj)
